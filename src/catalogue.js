...
function createCard(record, idx, catFolders, catNames, currentPageCode) {
  ...
  // currentPageCode is which PAGE this card is being rendered on. recordPageCode
  // is whatever page this artwork variant is tagged for in the catalogue —
  // irrelevant here, since the home page's featured grid can pull in any cc
  // regardless of recordPageCode, and using recordPageCode for the product
  // type would mislabel a sticker as a "Macbook Skin" just because that same
  // artwork is also sold as a skin elsewhere.
